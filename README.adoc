...
BEN: I don't follow this use case: 
...
This is effective in many situations, but can fall short in certain circumstances.
...
Using Vert.x without using this library with *ordered* processing requires dealing with the quite complicated handling of offset commits within Vert.x's asynchronous processing system.
+
...
More typically though you probably still want the per key ordering guarantees that Kafka provides.
